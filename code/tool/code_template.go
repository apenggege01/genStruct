...
const (
	...
	DicValueForServer = "\tMapRowsData map[%s] %s\n" // 解析出来的字典
	DicValueEnd       = "\n"                         // 结构体内容结束
	DicEnd            = "}\n"                        // 结构体结束
)
...
const (
	...
	InitBegin        = "func (this *%sDic)Init%sDic()(error){\n" // 初始化函数开始
	InitFilePath     = "\tthis.TableName = \"%s.csv\"\n"         // 表名
	TempMapRows      = "\tmapRowsData := make(map[%s]%s, 0)\n"   // 提取字典数据

	readCsv     = "\tcsv, err := NewWithOpts(this.TableName, %s{}, Comma(','), LazyQuotes(true), SkipLine(4))\n"
	readCsvRet  = "\tif err != nil {\n"
	readCsvRet1 = "\t\tthis.MapRowsData = make(map[%s]%s, 0)\n" // 提取字典数据
	readCsvRet2 = "\t\treturn err\n"
	readCsvRet3 = "\t}\n"
	...
	readCsvRet8   = "\t\t\treturn err\n"
	readCsvRet9   = "\t\t}\n"
	readCsvRet9_1 = "\t\tif _,ok:= mapRowsData[temp.Id]; ok{\n"
	readCsvRet9_2 = "\t\t\treturn errors.New(this.TableName + fmt.Sprintf(\" repeat key %v \", temp.Id))\n"
	readCsvRet9_3 = "\t\t}\n"
	readCsvRet10  = "\t\tmapRowsData[temp.Id] = temp\n"
	readCsvRet11  = "\t}\n"
	readCsvRet12  = "\tthis.MapRowsData  = mapRowsData\n"
	readCsvRet13  = "\treturn nil\n"
...
// 初始化方法
const (
	GetBegin     = "func (this *%sDic) Get%sCfgByID(id %s) (%s, error) {\n" // 查询函数开始
	GetDoGet     = "    cfg, isok := this.MapRowsData[id]\n"                // 提取字典数据
	GetIsOK      = "    if !isok {\n"                                       // 判断是否提取成功
	GetRetErrVal = "		return cfg, errors.New(\"rows not exist\")\n"         // 返回错误
	GetEnd       = "	}\n"                                                   //提取完成
	GetRetVal    = "	return cfg, nil\n"                                     // 返回错误
	GetFuncEnd   = "}\n"
)
...
const (
	...
	Init1   = "func (this *ConfigManager)InitAllConfig(csvRoot string) []error{\n" // 公共管理代码生成
	Init2   = "\terrList := make([]error, 0)\n"                                    // 公共管理代码生成
	Init3   = "\tSetCSVPath(csvRoot)\n"                                            // 公共管理代码生成
	Init3_1 = "\tvar err error\n"                                                  // 公共管理代码生成
	Init4   = "\terr = this.%sCfg.Init%sDic()\n"                                   // 公共管理代码生成
	Init5   = "\tif err != nil{\n"                                                 // 公共管理代码生成
	Init6   = "\t\terrList = append(errList, err)\n"                               // 公共管理代码生成
	Init7   = "\t}\n"                                                              // 公共管理代码生成
	Init8   = "\treturn errList"                                                   // 公共管理代码生成
	Init9   = "}\n"                                                                // 公共管理代码生成
)

const (
	ReloadConst1 = "func (this *ConfigManager)ReloadConfig(fileNameList []string) []error {\n"
	ReloadConst2 = "\terroList := make([]error, 0)\n"
	ReloadConst3 = "\tif fileNameList == nil || len(fileNameList) <= 0{\n"
	ReloadConst4 = "\t\treturn append(erroList, errors.New(\"ReloadConfig fileNameList is empty\"))\n"
	ReloadConst5 = "\t}\n"
	ReloadConst6 = "\tfor _,fileName := range fileNameList{\n"
	ReloadConst7 = "\t\tswitch fileName {\n"
	ReloadConst8 = "\t\t\tcase \"%s\" :\n"
	ReloadConst9 = "\t\t\t\terr := this.%sCfg.Init%sDic()\n"

	ReloadConst9_1 = "\t\t\t\tif err != nil{\n"                     // 公共管理代码生成
	ReloadConst9_2 = "\t\t\t\t\terroList = append(erroList, err)\n" // 公共管理代码生成
	ReloadConst9_3 = "\t\t\t\t}\n"                                  // 公共管理代码生成

	ReloadConst10 = "\t\t\tdefault:\n"
	ReloadConst11 = "\t\t\t\terroList = append(erroList, errors.New(fileName + \"file is not exist\"))\n"
	ReloadConst12 = "\t\t}\n"
	ReloadConst13 = "\t}\n"
	ReloadConst14 = "\treturn erroList\n"
	ReloadConst15 = "}\n"
)
...
func (this *Generate) GenerateStruct(readPath, savePath string) error {
	...
	for _, fileInfo := range files {
		...
		if openErr != nil {
			return fmt.Errorf("error %v  open file %s", openErr, readPath+"\\"+fileInfo.Name())
		}
		defer file.Close()
		...
		// 检查长度说明
		//if len(sheetData[TablelineType]) != len(sheetData[TablelineComment]) || len(sheetData[TablelineKey]) != len(
		//	sheetData[TablelineExpTyp]) || len(sheetData[TablelineType]) != len(sheetData[TablelineKey]) {
...
func (this *Generate) GenCommFile() error {
	...
	structData := fmt.Sprintf(ImportErro)
	...
	structData += fmt.Sprintf(Init3)
	structData += fmt.Sprintf(Init3_1)

	for _, structName := range this.FileNameSlice {
		structData += fmt.Sprintf(Init4, firstRuneToUpper(structName), firstRuneToUpper(structName))
		structData += fmt.Sprintf(Init5)
		structData += fmt.Sprintf(Init6)
		structData += fmt.Sprintf(Init7)
	}
	structData += fmt.Sprintf(Init8)
	structData += fmt.Sprintf(FormateChangeRow)
	structData += fmt.Sprintf(Init9)
	...
	for _, structName := range this.FileNameSlice {
		...
		structData += fmt.Sprintf(ReloadConst9_1)
		structData += fmt.Sprintf(ReloadConst9_2)
		structData += fmt.Sprintf(ReloadConst9_3)
	...
	err := this.WriteNewFile(structData, ConfigManagerFileName)
	if err != nil {
		return err
	}
...
func (this *Generate) SplicingData(primeKeyType string, data [][]string, structName string) error {
	...
	structData += fmt.Sprintf(DicValueForServer, primeKeyType, firstRuneToUpper(structName))
	...
	structData += fmt.Sprintf(TempMapRows, primeKeyType, firstRuneToUpper(structName))

	structData += fmt.Sprintf(readCsv, firstRuneToUpper(structName))
	structData += fmt.Sprintf(readCsvRet)
	structData += fmt.Sprintf(readCsvRet1, primeKeyType, firstRuneToUpper(structName))
	...
	structData += fmt.Sprintf(readCsvRet9_1)
	structData += fmt.Sprintf(readCsvRet9_2)
	structData += fmt.Sprintf(readCsvRet9_3)
	structData += fmt.Sprintf(readCsvRet10)
	structData += fmt.Sprintf(readCsvRet11)
	structData += fmt.Sprintf(readCsvRet12)
	structData += fmt.Sprintf(readCsvRet13)

	structData += fmt.Sprintf(InitEnd)

	// 提取字典数据方法
	structData += fmt.Sprintf(FormateChangeRow)
	structData += fmt.Sprintf(GetBegin, firstRuneToUpper(structName), firstRuneToUpper(structName), primeKeyType,
		firstRuneToUpper(structName))
...
func (this *Generate) CopytemplateFile() error {
	...
	files, err := ioutil.ReadDir("./" + TemplteFileName)
	if err != nil {
		return fmt.Errorf("ReadExcel|ReadDir is err:%v", err)
	}
	for _, fileInfo := range files {
...
// 根据类型类型标签获取类型名字
func getTypeNameByTypeTag(flag string) string {
	return ""
}
